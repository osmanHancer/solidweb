...
class Q_APP
{
   ...
   function start()
   {
      ...
      if (LANG_MODE) {
         if (strlen($paths[1]) !=2  && !empty(LANG_DEFAULT)) { //if home url redirect to default lang
            header('Location: /' . LANG_DEFAULT . '/'.(strlen($paths[1])>0?$paths[1].'/':'').(isset($_SERVER['QUERY_STRING'])?"?".$_SERVER['QUERY_STRING']:""));
            exit('301');
   ...
   public static  function error($code, $msg, $viewVars = null, $raw = true)
   {
